...
day: clean
	$(CL) -t atari -O --start-addr 0x3000 -Wl "-D__RESERVED_MEMORY__=0x3000" -o bin/solution src/day7.c 
clean:
	@$(DEL) bin/day*.* 2>$(NULLDEV)
dist: day 
	cp assets/disk.atr bin/aoc.atr
	tr '\12\11' '\233\177' < resources/data7.txt > bin/data.txt
...
test:
	$(CL) -t atari -O -o bin/test src/day7.c
	cp assets/disk.atr bin/test.atr
	tr '\12\11' '\233\177' < resources/test_data7.txt > bin/data.txt
