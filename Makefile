...
day1: clean
	$(CL) -t atari -O  -o bin/solution src/day1b.c 
clean:
	@$(DEL) bin/day*.* 2>$(NULLDEV)
dist: day1 
	cp assets/disk.atr bin/aoc.atr
	tr '\12\11' '\233\177' < resources/data1.txt > bin/data.txt
...
test:
	$(CL) -t atari -O  -o bin/test src/day1b.c
	cp assets/disk.atr bin/test.atr
	tr '\12\11' '\233\177' < resources/test_data.txt > bin/data.txt
